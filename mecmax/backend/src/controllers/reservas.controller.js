...
// 1. Listar Reservas (COM NOME DO MECÂNICO)
...
exports.criarReserva = async (req, res) => {
  try {
    ...
    if (!id_ferramenta) {
      ...
      if (!ferramentas || ferramentas.length === 0) return res.status(404).json({ success: false, message: "Ferramenta não encontrada." });
      id_ferramenta = ferramentas[0].id_ferramenta;
    }

    const sqlCheckLoan = `SELECT id_emprestimo FROM emprestimos WHERE id_ferramenta = ? AND status_emprestimo IN ('ATIVO', 'ATRASADO') AND ((data_retirada BETWEEN ? AND ?) OR (previsao_devolucao BETWEEN ? AND ?) OR (? BETWEEN data_retirada AND previsao_devolucao))`;
    ...
    if (loans && loans.length > 0) return res.status(400).json({ success: false, message: "Conflito: Ferramenta emprestada neste período." });

    const sqlCheckRes = `SELECT id_reserva FROM reservas WHERE id_ferramenta = ? AND status_reserva = 'ATIVA' AND ((data_reserva_inicio BETWEEN ? AND ?) OR (data_reserva_fim BETWEEN ? AND ?) OR (? BETWEEN data_reserva_inicio AND data_reserva_fim))`;
    ...
    if (reservasExistentes && reservasExistentes.length > 0) return res.status(400).json({ success: false, message: "Conflito: Já existe reserva neste horário." });

    const sqlInsert = `INSERT INTO reservas (id_mecanico, id_ferramenta, data_reserva_inicio, data_reserva_fim, status_reserva) VALUES (?, ?, ?, ?, 'ATIVA')`;
    ...
    return res.json({ success: true, message: "Reserva criada!", id_reserva: insertId });
  } catch (err) { return res.status(500).json({ success: false, message: "Erro ao processar." }); }
};
...
exports.cancelarReserva = async (req, res) => {
  try {
    const { id } = req.params;
    const consulta = await db.query("DELETE FROM reservas WHERE id_reserva = ?", [id]);
    const result = extrairDados(consulta);
    const affectedRows = result.affectedRows || (result[0] ? result[0].affectedRows : 0);
    if (affectedRows === 0) return res.status(404).json({ success: false, message: "Não encontrada." });
...
exports.buscarReserva = async (req, res) => {
  try {
    const { id } = req.params;
    const sql = `SELECT r.*, f.nome_ferramenta, f.codigo_ferramenta FROM reservas r JOIN ferramentas f ON r.id_ferramenta = f.id_ferramenta WHERE r.id_reserva = ?`;
...
exports.atualizarReserva = async (req, res) => {
  try {
    ...
    const consultaLoan = await db.query(sqlCheckLoan, [id_ferramenta, inicio, fim, inicio, fim, inicio]);
    const loans = extrairDados(consultaLoan);
    if (loans && loans.length > 0) return res.status(400).json({ success: false, message: "Conflito: Ferramenta emprestada." });

    const sqlCheckRes = `SELECT id_reserva FROM reservas WHERE id_ferramenta = ? AND status_reserva = 'ATIVA' AND id_reserva != ? AND ((data_reserva_inicio BETWEEN ? AND ?) OR (data_reserva_fim BETWEEN ? AND ?) OR (? BETWEEN data_reserva_inicio AND data_reserva_fim))`;
    ...
    if (colisoes && colisoes.length > 0) return res.status(400).json({ success: false, message: "Conflito: Reserva existente." });

    await db.query("UPDATE reservas SET data_reserva_inicio = ?, data_reserva_fim = ? WHERE id_reserva = ?", [inicio, fim, id]);
